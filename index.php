...
<body>  
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
    	...
    	if (empty($_POST["genkey"])) {
    		...
    		$key = md5(checkInput($_POST["genkey"]));
    		if ($key != '626726e60bd1215f36719a308a25b798' && !in_array($key, $keys)) {
	...
	function checkRegisterPlayer($conn,$player)
	{
		$sql = "SELECT * FROM player WHERE name ='$player' ORDER BY id DESC LIMIT 1";
		$result = $conn->query($sql);
		$register = 0;
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
	...
	function saveGenerateSchedule($conn, $team)
	{
		...
		$conn->query($sql);
		$sql = "UPDATE player SET register=0";
		$conn->query($sql);
	}
	// ---------------- //
					...
					<form method="post" class="form-inline">
						...
						<table class="table table-responsive table-bordered table-striped text-center summary">
							...
							<tbody>
								...
								<tr>
									<td><input type="checkbox" name="player[]" value="<?php echo $key ?>" <?php if (checkRegisterPlayer($conn,$key)) echo 'checked'; ?>/></td>
									<td><?php echo $p; ?></td>
									<td class="player"><?php echo getPlayer($key); ?></td>
